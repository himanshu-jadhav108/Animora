...
## Summary of Dependency Classifications

Animora categorizes its external software dependencies into five distinct tiers:

1. **Runtime Dependencies**: Required when running Animora as a Python library. Installed automatically via `pip install animora`.
2. **Optional Dependencies**: Enabled for specific extra functionality (e.g., CLI formatting, documentation building).
3. **Development Dependencies**: Used exclusively by maintainers and contributors for building, linting, type-checking, and running test suites.
4. **System Dependencies**: External operating system binaries or shared libraries invoked for media encoding and vector rasterization (not bundled into the Animora Python wheel).
5. **Bundled / Included Assets**: Files and templates directly included in the repository.

---

## 1. Runtime Dependencies

These packages are installed automatically as direct dependencies in `pyproject.toml`.
...
These dependencies are installed only when explicitly requested using pip extras:
...
These dependencies are used strictly during testing, static analysis, formatting, and package building:
...
The following external tools are system-level utilities executed by underlying rendering backends. They are **not** bundled or redistributed within the Animora Python package:
...
## 5. Bundled & Included Repository Assets

- **Documentation Media (`docs/assets/media/*.svg`)**: All visual diagrams, scene figures, and architecture SVGs are original works authored for Animora and licensed under the repository's [MIT License](LICENSE).
- **CLI Templates (`src/animora/cli/templates/*.py.template`)**: Starter scene templates provided for `animora new` scaffolding are original Animora code released under the [MIT License](LICENSE).
- **Vendored Code & Third-Party Assets**: Animora does not include vendored third-party source trees, proprietary fonts, or external binary blobs in its source tree or distribution packages.

---

## 6. Items for Downstream Review

> [!NOTE]
> - **Container & Appliance Distribution**: If downstream redistributors package Animora into self-contained Docker images or appliances that bundle FFmpeg binaries compiled with non-free or GPL-licensed codecs (e.g., `libx264`), they should ensure compliance with the respective FFmpeg build license terms.
